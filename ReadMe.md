...
# 为生产环境写入正确的 API 地址（建议使用与前端同域的反向代理路径）
cat <<'EOF' > .env.production
VITE_API_BASE_URL=https://tire.example.com/api
EOF
...
- 若前后端同域部署（如通过 Nginx 把 `/api` 反向代理到后端），也可以将 `VITE_API_BASE_URL` 设为相对路径 `"/api"`，避免跨域问题。
- 开发模式可运行：
  ```bash
  npm run dev -- --host 0.0.0.0 --port 5173
  ```
- 默认前端通过 `VITE_API_BASE_URL` 环境变量（默认 `http://localhost:8000`）调用后端，生产构建前请根据部署域名调整该值。
